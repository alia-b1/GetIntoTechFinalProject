...
//      $list = []; 
//      $db = Db::getInstance(); 
//      $req = $db->query('SELECT * FROM blog_user WHERE user_type_id = :2'); 
...
//    } should only be for admin 
    ...
    public static function find($id) { 
      ...
      $id = intval($id); 
      $req = $db->prepare('SELECT * FROM blog_user WHERE id = :id, user_type_id = 3'); 
      //the query was prepared, now replace :id with the actual $id value 
      $req->execute(array('id' => $id)); 
      $reader = $req->fetch(); 
if($reader){ 
      return new Reader($reader['first_name'], $reader['last_name'], $reader['date_joined']); 
    } 
    else 
    { 
